...
def is_setup_completed():
    return os.path.exists(setup_completed_flag)

# Mark setup as completed
def mark_setup_completed():
    with open(setup_completed_flag, 'w') as f:
        f.write("Setup completed")

# Run webull_setup.py if setup is not completed
if not is_setup_completed():
    subprocess.run(['python', webull_setup_path])
...
# If configuration is incomplete, prompt the user for inputs
if not config:
    config['robinhood_file_path'] = input("Enter the file path for Robinhood credentials: ")
    config['public_file_path'] = input("Enter the file path for Public credentials: ")
    config['webull_file_path'] = input("Enter the file path for Webull credentials: ")
    config['firstrade_file_path'] = input("Enter the file path for Firstrade credentials: ")
    config['tradier_file_path'] = input("Enter the file path for Tradier credentials: ")
    config['fennel_file_path'] = input("Enter the file path for Fennel credentials: ")

    config['discord_token'] = input("Enter your Discord bot token: ")
    config['buy_channel_id'] = int(input("Enter your Discord buy channel ID: "))
    config['sell_channel_id'] = int(input("Enter your Discord sell channel ID: "))
    config['alerts_channel_id'] = int(input("Enter your Discord alerts channel ID: "))
    config['command_channel_id'] = int(input("Enter your Discord command channel ID: "))

    # Save the configuration to a file
    save_config(config)

# Use the configuration
robinhood_file_path = config['robinhood_file_path']
public_file_path = config['public_file_path']
webull_file_path = config['webull_file_path']
firstrade_file_path = config['firstrade_file_path']
tradier_file_path = config['tradier_file_path']
fennel_file_path = config['fennel_file_path']
...
public_json_file_path = os.path.join(script_dir, 'currentArbsPublic.json')
webull_json_file_path = os.path.join(script_dir, 'currentArbsWebull.json')
firstrade_json_file_path = os.path.join(script_dir, 'currentArbsFirstrade.json')
tradier_json_file_path = os.path.join(script_dir, 'currentArbsTradier.json')
fennel_json_file_path = os.path.join(script_dir, 'currentArbsFennel.json')
holidays_json_file_path = os.path.join(script_dir, 'market_holidays.json')
...
async def on_ready():
    ...
    try:
        ...
        print("✅ Logged in successfully to Fennel!")
        embed.add_field(name="Fennel", value="✅ Logged in successfully to Fennel!", inline=False)
    except Exception as e:
        print(f"❌ Fennel login failed: {e}")
        embed.add_field(name="Fennel", value=f"❌ Fennel login failed: {e}", inline=False)
...
async def on_message(message):
    if message.channel.id == buy_channel_id and message.content.startswith('$'):
        ...
        try:
            fennel_result = buy_stock_fennel(ticker)
        except Exception as e:
            print(f"Failed to place order for {ticker} on Fennel: {e}")

        await send_order_message(message.channel, ticker, robinhood_result, public_result, webull_result, firstrade_result, tradier_result, fennel_result)
...
def buy_stock_fennel(ticker):
    try:
        fennel = Fennel()
        fennel.login(email=fennel_email, wait_for_code=True, code=None)
        order_result = fennel.place_order(
            account_id=fennel_account_id,
            ticker=ticker,
            quantity=1,
            side="buy",
            price="market",
        ...
        print(f"Fennel order result: {order_result}")  # Debug log
        if order_result.get('data', {}).get('createOrder') == 'pending':
            tickers = read_tickers(fennel_json_file_path)
            tickers.append(ticker)
            write_tickers(fennel_json_file_path, tickers)
            return {"success": True}
        elif order_result.get('data', {}).get('createOrder') == 'error':
            return {"success": False, "detail": "Order failed with error status"}
        else:
...
async def send_order_message(channel, ticker, robinhood_result, public_result, webull_result, firstrade_result, tradier_result, fennel_result):
    ...
    fennel_status = "✅" if fennel_result and fennel_result.get('success', False) else f"❌ Fennel: {fennel_result.get('detail', 'Unknown error') if fennel_result else 'Unknown error'}"
    ...
    embed.add_field(name="🍃 Fennel", value=fennel_status, inline=False)
...
def buy_SCHG_tradier():
    ...
    try:
        ...
        money_needed_for_SCHG = money_for_SCHG - 15
...
def buy_SCHG_fennel():
    try:
        fennel = Fennel()
        fennel.login(email=fennel_email, wait_for_code=True, code=None)
        buying_power = get_buying_power_fennel()
        schg_price = get_stock_price("SCHG") + 5

        if buying_power >= schg_price:
            order_result = fennel.place_order(
                account_id=fennel_account_id,
                ticker="SCHG",
                quantity=1,
...
def sell_all_shares_fennel():
    try:
        ...
        fennel = Fennel()
        fennel.login(email=fennel_email, wait_for_code=True, code=None)
        positions = fennel.get_stock_holdings(fennel_account_id)

        for ticker in tickers:
            try:
                position = next((p for p in positions if p['security']['ticker'] == ticker), None)
                if position:
                    shares = float(position['investment']['ownedShares'])
                    last_price = float(position['investment']['marketValue']) / shares
                    order_result = fennel.place_order(
                        account_id=fennel_account_id,
                        ticker=ticker,
                        quantity=shares,
                    ...
                    if order_result.get('data', {}).get('createOrder') == 'pending':
                        profit = shares * last_price
                        total_profit += profit
                        message = f"Sold {shares} shares of {ticker} at ${last_price:.2f}."
                        result_messages.append(message)
                        tickers_to_remove.append(ticker)
                    else:
                        raise ValueError(f"Failed to sell shares of {ticker}. Order result: {order_result}")
                ...
                error_message = f"Failed to sell shares of {ticker} on Fennel: {e}"
                result_messages.append(error_message)

        remaining_tickers = [ticker for ticker in tickers if ticker not in tickers_to_remove]
        write_tickers(fennel_json_file_path, remaining_tickers)
        ...
        return f"Failed to process Fennel shares: {e}", total_profit
...
async def sell_all_shares_discord():
    if datetime.today().weekday() < 5:
        ...
        fennel_message, fennel_profit = sell_all_shares_fennel()

        total_profit = robinhood_profit + public_profit + webull_profit + firstrade_profit + tradier_profit + fennel_profit
        ...
        embed.add_field(name="🍃 Fennel", value=fennel_message, inline=False)
...
async def buy_VUG():
    if datetime.today().weekday() < 5:
        ...
        try:
            fennel_result = buy_SCHG_fennel()
        except Exception as e:
            print(f"Failed to buy SCHG on Fennel: {e}")
            fennel_result = {"success": False, "detail": str(e)}
        ...
        embed.add_field(name="Fennel - SCHG", value=f"{'✅' if fennel_result['success'] else '❌'} - {fennel_result.get('balance', fennel_result.get('detail', 'Error'))}", inline=False)
...
def get_buying_power_fennel():
    try:
        fennel = Fennel()
        fennel.login(email=fennel_email, wait_for_code=True, code=None)
        portfolio = fennel.get_portfolio_summary(fennel_account_id)
        cash_balance = portfolio['cash']['balance']['canTrade']
        return float(cash_balance)
    except Exception as e:
        print(f"Failed to get buying power from Fennel: {e}")
        return 'x'

@bot.tree.command(name="json", description="View the ticker symbols from all or a specific brokerage's JSON file")
@app_commands.describe(brokerage="Select a brokerage to view its tickers")
async def json_command(interaction: discord.Interaction, brokerage: str = None):
    ...
    brokerages = {
        ...
        "Fennel": fennel_json_file_path
    }
    
    if brokerage:
        brokerage = brokerage.capitalize()
        if brokerage in brokerages:
            tickers = read_tickers(brokerages[brokerage])
            embed = discord.Embed(title=f"Stocks in {brokerage} JSON", color=0xAEA04B)
            embed.add_field(name=brokerage, value="\n".join(tickers) if tickers else "No stocks found", inline=False)
        else:
            embed = discord.Embed(title="Error", description="Invalid brokerage specified.", color=0xAEA04B)
    else:
        embed = discord.Embed(title="Stocks in JSON Files", color=0xAEA04B)
        for key, path in brokerages.items():
            tickers = read_tickers(path)
            embed.add_field(name=key, value="\n".join(tickers) if tickers else "No stocks found", inline=False)
    
    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="add", description="Add a ticker symbol to all or a specific brokerage's JSON file")
@app_commands.describe(ticker="The ticker symbol to add", brokerage="Select a brokerage to add the ticker to")
async def add_command(interaction: discord.Interaction, ticker: str, brokerage: str = None):
    ticker = ticker.upper()
...
@bot.tree.command(name="remove", description="Remove a ticker symbol from all or a specific brokerage's JSON file")
@app_commands.describe(ticker="The ticker symbol to remove", brokerage="Select a brokerage to remove the ticker from")
async def remove_command(interaction: discord.Interaction, ticker: str, brokerage: str = None):
    ticker = ticker.upper()
    brokerage = brokerage.capitalize() if brokerage else None
    brokerages = {
        "Robinhood": robinhood_json_file_path,
...
async def total(ctx):
    if ctx.channel.id != command_channel_id:
        await ctx.send(f"This command can only be used in the designated command channel: <#{command_channel_id}>", delete_after=10)
        return
    ...
    try:
        ...
        fennel.login(email=fennel_email, wait_for_code=True, code=None)
        positions = fennel.get_stock_holdings(fennel_account_id)
        SCHG_value = sum(float(pos['investment']['marketValue']) for pos in positions if pos['security']['ticker'] == 'SCHG')
        total_value += SCHG_value
        total_message += f"{emojis['Fennel']} Fennel: ${SCHG_value:.2f} - SCHG\n"
        print(f"Fennel SCHG value: ${SCHG_value:.2f}")
    except Exception as e:
        total_message += f"{emojis['Fennel']} Fennel: Error fetching data ({e})\n"
        print(f"Error fetching Fennel data: {e}")
...
async def portfolio(ctx, brokerage: str = None):
    ...
    def get_fennel_portfolio():
        from fennel_invest_api import Fennel
        try:
            fennel = Fennel()
            fennel.login(email=fennel_email, wait_for_code=True, code=None)
            portfolio = fennel.get_portfolio_summary(fennel_account_id)
            cash = float(portfolio['cash']['balance']['canTrade'])
            positions = fennel.get_stock_holdings(fennel_account_id)
            stock_details = []
            arbitrage_stocks = []
            total_stock_value = 0
            arbitrage_tickers = set(read_tickers(fennel_json_file_path))

            for pos in positions:
                try:
                    ticker = pos['security']['ticker']
                    quantity = float(pos['investment']['ownedShares'])
                    price = float(pos['security']['currentStockPrice'])
                    stock_value = quantity * price
                    if ticker == 'SCHG' or ticker in arbitrage_tickers:
                        if ticker == 'SCHG':
                            stock_details.append(f"{ticker}: {quantity} shares @ ${price:.2f} = ${stock_value:.2f}")
                    ...
                    print(f"Error processing ticker {ticker} in Fennel: {e}")

            total_value = cash + total_stock_value
            return cash, stock_details, arbitrage_stocks, total_value
        except Exception as e:
            print(f"Failed to get Fennel portfolio: {e}")
            return 0, [], [], 0
    ...
    if brokerage:
        ...
        if brokerage in brokerages:
            ...
            await ctx.send("Invalid brokerage specified. Please choose from: Robinhood, Public, Webull, Firstrade, Tradier, Fennel.")
            return
    else:
...
async def profit(ctx, brokerage: str = None):
    ...
    initial_investments = {
        ...
        'Fennel': float(11)  # Replace with actual initial investment
    }
    ...
    def get_fennel_portfolio():
        ...
        try:
            ...
            for pos in positions:
                ticker = pos['security']['ticker']
                quantity = float(pos.get('investment')['ownedShares'])
                price = float(pos.get('security')['currentStockPrice'])
                stock_value = quantity * price
                if ticker == 'SCHG' or ticker in arbitrage_tickers:
                    total_value += stock_value
...
@bot.tree.command(name="buy_fennel", description="Buy a stock on Fennel")
@app_commands.describe(ticker="The ticker symbol to buy")
async def buy_fennel_command(interaction: discord.Interaction, ticker: str):
    if interaction.channel.id != command_channel_id:
        await interaction.response.send_message(f"This command can only be used in the designated command channel: <#{command_channel_id}>", ephemeral=True)
        return

    await interaction.response.send_message(f"Initiating buy for {ticker} on Fennel...")
    result = buy_stock_fennel(ticker)
    if result.get('success', False):
        await interaction.edit_original_response(content=f"Completed buy for {ticker} on Fennel.")
    else:
        await interaction.edit_original_response(content=f"Failed to buy {ticker} on Fennel: {result.get('detail', 'Unknown error')}")
...
async def sell_all_stocks_command(interaction: discord.Interaction, brokerage: str = None):
    ...
    if brokerage:
        ...
        elif brokerage == "fennel":
            message, _ = sell_all_shares_fennel()
            messages.append(f"Fennel:\n{message}")
        else:
...
# Function to get the local time zone
...
async def schedule_tasks():
    ...
    # Convert 8:45 AM CST to local time
    cst_time_sell = cst.localize(datetime.strptime('08:45', '%H:%M')).astimezone(local_tz)
    local_time_sell = cst_time_sell.strftime('%H:%M')

    # Convert 9:00 AM CST to local time
    cst_time_buy = cst.localize(datetime.strptime('09:00', '%H:%M')).astimezone(local_tz)
    local_time_buy = cst_time_buy.strftime('%H:%M')

    schedule.every().day.at(local_time_sell).do(lambda: asyncio.create_task(sell_all_shares_discord()))
    schedule.every().day.at(local_time_buy).do(lambda: asyncio.create_task(buy_VUG()))
    while True:
